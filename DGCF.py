    ...
    for j in range(train_end_idx):
        ...
        if timestamp - tbatch_start_time > tbatch_timespan:
            ...
            for i in range(len(lib.current_tbatches_user)):
                ...
                Fea = lib.current_tbatches_feature[i]
                for k in Fea:
                    if len(k)==0:
                        k.append(0.0)
                        k.append(0.0)

                feature_tensor = Variable(torch.Tensor(Fea).cuda()) # Recall "lib.current_tbatches_feature[i]" is list of list, so "feature_tensor" is a 2-d tensor
