...
parser.add_argument('--dir_path',default='../Esun_data/result/week_no_feature', help='Directory of the embedding file store')
parser.add_argument('--method', default="Node2vec", help='Model name')
...
def scores_then_recall(filename, test, recall_num, user_node, user_start, method = None):
    S = [0 for i in xrange(len(recall_num))]
    Prec = [0 for i in xrange(len(recall_num))]
    ...
    for txt in filename:
        ...
        D = {}
        for i in xrange(len(df1[0])):
            D[df1[0][i]] = list(df1.iloc[i,1:])
            
        # user-item inner product
        items_users = list(df1[0])
        #print(items_users)
        items = [int(ele) for ele in items_users if int(ele) not in [user_start +j for j in xrange(user_node)]]
        scores = [[] for i in xrange(user_node)]

        for i in xrange(len(items)):
            for k in xrange(user_node):
        ...
        rec_list = [0 for i in xrange(user_node)]
        Max_scores = [0 for i in xrange(user_node)]
        for k in xrange(user_node):
            test_list = scores[k]
            res = sorted(xrange(len(test_list)), key = lambda sub: test_list[sub])[-len(test_list):]
            rec_list[k] = [items[i] for i in res]
            Max_scores[k] = [test_list[i] for i in res]
            
        items_users = list(df1[0])
        items = [ele for ele in items_users if ele not in [user_start +j for j in xrange(user_node)]]
        test_item = list((test['item_id']))

        rec_list = [rec_list[i][::-1] for i in xrange(len(rec_list))]

        # MRR
        MRR=0
        for i in xrange(len(test_item)):
            for idx in xrange(user_node):
                if list(test['user_id'])[i]==idx and test_item[i] in rec_list[idx]:
                    place = rec_list[idx].index(test_item[i])
                    MRR += 1/(place+1)
        MRR_all += MRR/len(test_item)
        
        for num in xrange(len(recall_num)):
            # Recall@num
            s = 0
            for i in xrange(len(test_item)):
                for idx in xrange(user_node):
    ...
    for i in xrange(len(recall_num)):
        fw.write('\nAverage rec@'+str(recall_num[i])+' of '+str(len(filename))+' result : '+ str(S[i]))
        fw.write('\nAverage prec@'+str(recall_num[i])+' of '+str(len(filename))+' result : '+str(Prec[i]))
        print('Average rec@'+str(recall_num[i])+' of '+str(len(filename))+' result :',str(S[i]))
        print('Average prec@'+str(recall_num[i])+' of '+str(len(filename))+' result :',str(Prec[i]))
    
    fw.write('\nMRR : '+ str(MRR_all))
    print('MRR result :',MRR_all)
